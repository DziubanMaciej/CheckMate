...
// impl ThreadCommunicationThreadSafe {
//     pub fn get_access(&self) -> MutexGuard<ThreadCommunication> {
...
impl ThreadCommunication {
    ...
    pub fn read_messages(&self, receiver: &mpsc::Receiver<ThreadMessage>) -> Vec<String> {
        let mut data: PerThreadDataMap;
        {
            ...
            let mut lock = self.locked_data.lock().unwrap();
            let mut original_data = lock.deref_mut();
